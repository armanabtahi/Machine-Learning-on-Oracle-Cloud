...
def main():
    ...
    with pool.acquire() as connection_adw:
        with connection_adw.cursor() as cursor:
            try:
                # create the table
                cursor.execute("""CREATE TABLE Crypto (timestamp NUMBER PRIMARY KEY,
                                                            rate NUMBER, 
                                                            volume NUMBER, 
                                                            cap NUMBER, 
                                                            liquidity NUMBER)""")
    ...
    for obj in list_objects_response:
        if obj.name[-7:] == "parquet":
            ...
            df = df.fillna('')
            if not df.empty:
                data += df[['pubDate', 'title', 'link', 'keywords', 'creator','video_url','description','content','image_url','source_id','category','country','language']].values.tolist()
        ...
        with connection_adw.cursor() as cursor:
            try:
                ...
                cursor.executemany("""
                        ...
                        WHEN NOT MATCHED THEN
                            INSERT (pubDate, title, link, keywords,creator,video_url,description,content,image_url,source_id, category,country, language) 
                            VALUES (d.pubDate, d.title, d.link, d.keywords,d.creator,d.video_url,d.description,d.content,d.image_url,d.source_id, d.category,d.country, d.language)""",
